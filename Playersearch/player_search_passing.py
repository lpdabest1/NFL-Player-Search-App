...
def app():
    ...
    rows = pd.read_csv('CSV_Files/NFL_QB/NFL_QB_Search.csv')
    player_img = pd.read_csv('CSV_Files/NFL_QB/NFL_QB_Search_Images.csv')
    ...
    if not qb_img.Player.isin([user_input]).any():
        qb_img_placeholder = Image.open('Placeholder_Images/qb_playerholder_img.jpg')
    ...
    if user_input and player_index >= 0:
        col1.success(user_input + ' ranked ' + str(player_rank) + ' out of ' + str(ranking_len) + ' qualifying Quarterbacks during the ' + str(selected_years) + ' season.')
