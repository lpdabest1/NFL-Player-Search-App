...
from nfl_player_search.config import CATEGORIES, TEAM_COLORS, CategoryConfig
...
def render_category_page(config: CategoryConfig) -> None:
    ...
    choice = st.segmented_control(
        ...
        key=f"category_segmented_{config.key}",
        label_visibility="collapsed",
    )
    if choice is not None and choice != config.label:
        st.switch_page(_CATEGORY_PAGE_PATHS[choice])

    st.sidebar.header(config.label)

    @st.fragment
    def _player_explorer() -> None:
        """Player/year filters + results; fragment reruns without full-app chart reload side effects."""
        stats, images = load_category_frames(config)
        players = sorted(stats["Player"].dropna().unique().tolist())
        player = st.selectbox("Select a player to search", players, key=f"player_{config.key}")

        years = list(reversed(player_years(stats, player)))
        if not years:
            st.warning("No season rows found for this player.")
            return
        year = st.selectbox("Select a Year", years, key=f"year_{config.key}")
        ...
        if team not in TEAM_COLORS:
            if st.sidebar.checkbox("Custom Color", key=f"custom_color_{config.key}"):
                color = st.sidebar.color_picker(
                    "Pick a custom color for player chart",
                    value="#1f77b4",
                    key=f"color_picker_{config.key}",
                )
                st.sidebar.info(
        ...
        if st.sidebar.checkbox("Career", key=f"career_{config.key}"):
        ...
        if st.sidebar.checkbox(
            f"{config.season_checkbox_label} {year} Season",
            key=f"season_stats_{config.key}",
        ):
            st.caption(config.season_caption)
            st.dataframe(leaders, width="stretch")

        if st.sidebar.checkbox("Rankings", key=f"rankings_{config.key}"):
